...
int main(int argc, char** argv)
{
  try{
    ...
    config_parser.readINITree("unfitted_configs.ini", config_tree);
    ...
    if(write_output) {
      ...
      volume_writer_ptr->addVertexDataGradient(*tdcsSolution, "gradient_potential");
      ...
      for(size_t i = 0; i < nrElements; ++i) {
        ...
        for(size_t j = 0; j < dim; ++j) {
          elementCenters[i][j] = (*element_statistics)(i, j);
